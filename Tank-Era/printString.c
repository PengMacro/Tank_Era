...
{
	int i;
	uint8_t temp_character[32];
	uint16_t temp_line; // this is used to store 16 bits, namely each line
	uint8_t shift_right; // the first bytes
	uint8_t left_three_bits;//the second bytes
	int num_of_charactor = (int)character - 32;//offset
	const uint8_t* my_character;//this is indicating the memory that stores the character we want to print out
	my_character =(courierNewBitmap + 32 * num_of_charactor);//should be just 32 bytes
	lcd_set_pos(X_pixel,X_pixel+FONT_WIDTH-1,Y_pixel,Y_pixel+FONT_HEIGHT-1);//set the screen
	for(i = 0; i < 32; i++){
		uint8_t curr_byte = my_character[31-i];
		//Below is the process of reversing the byte
	  curr_byte = (curr_byte & 0xF0) >> 4 | (curr_byte & 0x0F) << 4;
    curr_byte = (curr_byte & 0xCC) >> 2 | (curr_byte & 0x33) << 2;
    curr_byte = (curr_byte & 0xAA) >> 1 | (curr_byte & 0x55) << 1;
		temp_character[i] = curr_byte;
	}
	//This for loop is used to trim each line by 16 bits together and then seperate 16 bit into 2 bytes
	for(i = 0; i < 32; i++){
		temp_line = (temp_character[i]<<8) | temp_character[i+1];
		temp_line = temp_line <<5;
		shift_right = (uint8_t)(temp_line >>8);
		temp_character[i] = shift_right;
		left_three_bits = (uint8_t)((temp_line<<8)>>8);
		temp_character[i+1] = left_three_bits;
		i++;
	}
	//actually drawing the charactor
	lcd_draw_image(
	X_pixel,
	FONT_WIDTH,
	Y_pixel,
...
{
	//start from X_PADDING and print
	int i;
	int char_length = strlen(msg);
	int x_pos;
	int y_pos;
	uint16_t X_pixel;
	uint16_t Y_pixel;
	//This is the upper left corner of the character
	x_pos =X;//the actual pos of the character- X
	y_pos =Y;//the actual pos of the character- Y
	for(i = 0; i < char_length; i++){
		//if print at the end of the row, change to the next one and start from X
		if(x_pos >= CHAR_COLUMNS){
			x_pos = X;
			y_pos++;
		}
		//if print at the end of the column, change to the first row and start from X
		if(y_pos >= CHAR_ROWS){
			y_pos = 0;
		}
		//if the first character is ' ' we move to the next character
		if(msg[i]==' '&&x_pos == X){
			continue;
		}
		//before passing in the X and Y we need to slightly change the pos to center of the character
		X_pixel = ((COLS % FONT_WIDTH)/2) + x_pos *FONT_WIDTH + FONT_WIDTH/2;
		Y_pixel = y_pos * FONT_HEIGHT +FONT_HEIGHT/2;
		//print the character
		lcd_print_character(X_pixel,Y_pixel,fg_color,bg_color,msg[i]);
		x_pos++;
	}
}
