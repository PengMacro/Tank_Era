...
void Timer0A_Config(uint16_t ticks, uint32_t mode, bool count_up, bool enable_interrupts){
  
	// CTL,CFG,TAMR,TAILR,ICR,IMR(since use interrupt),CTL
	TIMER0->CTL &= ~(TIMER_CTL_TAEN);
	TIMER0->CFG = TIMER_CFG_16_BIT;
	
	// set mode
	TIMER0->TAMR &= ~(TIMER_TAMR_TAMR_M); // clear mode before set
	TIMER0->TAMR |= mode; // one-shot or periodic
	if(count_up){
	...
	TIMER0->TAILR = ticks;
	TIMER0->ICR |= TIMER_ICR_TATOCINT;// why need? handled after handler????????????????????
	
	TIMER0->TAPR = 9;
	
	// use interrupt instaed of polling from RIS
	if(enable_interrupts){
...
void Timer0B_Config(uint16_t ticks, uint32_t mode, bool count_up, bool enable_interrupts){
	// CTL,CFG,TAMR,TAILR,ICR,IMR(since use interrupt),CTL
	TIMER0->CTL &= ~(TIMER_CTL_TBEN);
	TIMER0->CFG = TIMER_CFG_16_BIT;
	
	// set mode
	TIMER0->TBMR &= ~(TIMER_TBMR_TBMR_M); // clear mode before set
	TIMER0->TBMR |= mode; // one-shot or periodic
	if(count_up){
		TIMER0->TBMR |= TIMER_TBMR_TBCDIR;
	} else{
	  TIMER0->TBMR &= ~TIMER_TBMR_TBCDIR;
	}
	
	TIMER0->TBILR = ticks;
	TIMER0->ICR |= TIMER_ICR_TBTOCINT;// why need? handled after handler????????????????????
...
// get the mask for timer and enable it, set NVIC; call Timer0A_Config and Timer0B_Config to set mode
bool gp_timer_config_16(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts)
{
  ...
  timer0 = (TIMER0_Type *)base_addr;
	...
	Timer0A_Config(50000, mode, count_up, enable_interrupts);
	Timer0B_Config(50000, mode, count_up, enable_interrupts);
